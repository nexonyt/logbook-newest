...
import moment from "moment"; // jeśli chcesz formatować czas
...
const SectionTitle = styled.h2`
  ...
  font-weight: 600;
  font-size: 1.5rem;
  border-bottom: 2px solid #89216b;
...
export default function Stats() {
    ...
    useEffect(() => {
        const fetchStats = async () => {
            try {
                setLoading(true);
                const res = await fetch("http://localhost:4040/getflightdurationsum", {
                    method: "POST",
                    headers: {
          ...
          <Title>Statystyki lotów</Title>
                ...
                <Section>
                    ...
                    {stats.max_delay[0] ? (
                        <>
                            <p>
                                <Label>Numer lotu:</Label> <Value>{stats.max_delay[0].fli_number}</Value>
                            </p>
                            <p>
                                <Label>Linia lotnicza:</Label> <Value>{stats.max_delay[0].max_delay}</Value>
                            </p>
                            <p>
                                <Label>Opóźnienie:</Label> <Value>{stats.max_delay[0].fli_delay}</Value>
                            </p>
                            <p>
                                <Label>Lotnisko wylotu (ICAO/IATA):</Label>{" "}
                                <Value>
                                    {stats.max_delay[0].fli_dest_air_icao} / {stats.max_delay[0].fli_dest_air_iata}
                                </Value>
                            </p>
                            <p>
                                <Label>Lotnisko przylotu (ICAO/IATA):</Label>{" "}
                                <Value>
                                    {stats.max_delay[0].fli_arr_air_icao} / {stats.max_delay[0].fli_arr_air_iata}
                                </Value>
                            </p>
                        </>
                    ) : (
                ...
                <Section>
                    ...
                    <List>
                        {stats.most_frequent_departure_airport.map((airport, i) => (
                            <ListItem key={i}>
                                <Label>
                                    {airport.fli_dest_air_icao} / {airport.fli_dest_air_iata}
                                </Label>
                ...
                <Section>
                    ...
                    {stats.most_frequent_destination[0] ? (
                        <p>
                            ...
                            <Value>{stats.most_frequent_destination[0].count || stats.most_frequent_destination[0].fli_arr_air_iata}</Value>
                        </p>
                    ) : (
                        <p>Brak danych</p>
                    )}
